<?php
    ...
    array_key_exists('password',$_POST)) {
        ...
        if ($_POST['username']=='') {
            ...
            $cifrada = md5($contra);
            $query = "SELECT usuarios.username, usuarios.password, roles.rol FROM usuarios, roles WHERE username='".mysqli_real_escape_string($enlace,$_POST['username'])."' AND password='".$cifrada."' AND usuarios.rol = roles.id_rol LIMIT 1";
            ...
            if (mysqli_num_rows($result)>0){
                ...
                echo $_COOKIE['id'];
                if ($rolmayus=="ADMINISTRADOR") {
                    $enlace2 = mysqli_insert_id($enlace)+1;
                    setcookie("administrador",$enlace2,time()+60*60*24*365);
                    echo $_COOKIE['administrador'];
                }
