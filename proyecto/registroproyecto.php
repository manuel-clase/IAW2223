    ...
    array_key_exists('password',$_POST)) {
        ...
        if ($_POST['username']=='') {
            ...
            if (mysqli_num_rows($result)>0) {
                ...
                $cifrada = md5($contra);
                $query="INSERT INTO usuarios (username, password, rol, email) VALUES('".mysqli_real_escape_string($enlace,$_POST['username'])."','".$cifrada."',4,'".mysqli_real_escape_string($enlace,$_POST['email'])."')";
                if (mysqli_query($enlace,$query)){
                    $casilla = $_POST["keep"];
                    /*if ($casilla == "marcada") {
                        setcookie("id",mysqli_insert_id($enlace),time()+60*60*24*365);
                        echo "https://iawmanuelarcos-com.stackstaging.com/proyecto/paginaprincipal.php";
                    }*/
